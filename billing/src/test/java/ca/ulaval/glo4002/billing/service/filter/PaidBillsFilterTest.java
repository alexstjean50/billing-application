...
public class PaidBillsFilterTest
{
    ...
    private static final Instant SOME_OVERDUE_EFFECTIVE_DATE = Instant.now()
            .minusSeconds(20);
    private static final int PAID_BILLS_COUNT = 1;
    private final List<Bill> billsToFilter = new ArrayList<>();
    private final PaidBillsFilter paidBillsFilter = new PaidBillsFilter();
    ...
    @Test
    public void whenFilteringPaidBills_thenShouldOnlyHavePaidBills()
    {
        List<Bill> filteredBills = this.paidBillsFilter.filter(this.billsToFilter);
    ...
    @Test
    public void whenFilteringPaidBills_thenShouldKeepAllPaidBills()
    {
        List<Bill> filteredBills = this.paidBillsFilter.filter(this.billsToFilter);
        ...
        assertEquals(PAID_BILLS_COUNT, paidBills.size());
    }
